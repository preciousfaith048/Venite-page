...
export default function Programmes({ onOpenApplyModal }: ProgrammesProps) {
  ...
  return (
    <div className="relative pt-24">
      ...
      <section id="programme-directory" className="py-24 bg-white relative">
            ...
            <AnimatePresence mode="popLayout">
              {filteredProgrammes.map((p) => (
                <motion.div
                  ...
                  key={p.id}
                  className="bg-slate-50 border border-slate-100/80 rounded-2xl p-6 flex flex-col justify-between hover:shadow-md hover:bg-white hover:border-brand-100 transition-all duration-300 group"
                >
                  <div className="space-y-4">
                    {/* Header Details */}
                    <div className="flex items-start justify-between">
                      <div className="h-10 w-10 rounded-xl bg-brand-50 text-brand-600 flex items-center justify-center shrink-0 group-hover:bg-brand-600 group-hover:text-white transition-colors duration-300">
                        <LucideIcon name={getFacultyIconName(p.faculty)} size={20} />
                      </div>
                      <span className="font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-100/60 px-2 py-1 rounded">
                        {p.duration}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="font-mono text-[9px] font-bold text-brand-600 bg-brand-50 px-2 py-0.5 rounded uppercase tracking-wider">
                        {p.faculty}
                      </span>
                      <span className="font-mono text-[9px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded uppercase tracking-wider">
                        {p.degreeType}
                      </span>
                    </div>

                    <h3 className="font-sans font-bold text-lg text-slate-900 group-hover:text-brand-800 transition-colors">
                      {p.name}
                    </h3>
                  ...
                  <div className="pt-6 mt-4 border-t border-slate-100/80">
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl font-bold text-sm text-center text-brand-700 bg-brand-50 hover:bg-brand-600 hover:text-white transition-all duration-300 cursor-pointer"
                    >
                      Apply for Course
                    </button>
                  </div>
      ...
      <section id="admissions-guide" className="py-24 bg-slate-50 border-y border-slate-100 relative">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-xs">
              {[
                ...
                <div key={idx} className="flex items-start space-x-3 text-slate-700">
                  <div className="h-6 w-6 rounded-full bg-brand-50 text-brand-600 flex items-center justify-center shrink-0 mt-0.5">
                    <LucideIcon name="Check" size={14} />
                  </div>
                  <div>
                    <h4 className="font-sans font-bold text-slate-900 text-sm md:text-base">{req.title}</h4>
                    <p className="text-slate-500 text-xs md:text-sm mt-0.5 leading-relaxed">{req.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6 relative border-l-2 border-brand-100 ml-4 py-2">
              {[
                ...
                <div key={idx} className="relative pl-8 group">
                  <div className="absolute -left-[9px] top-1 h-4 w-4 rounded-full border-2 border-white bg-brand-600 shadow-[0_0_8px_rgba(2,132,199,0.5)] group-hover:scale-125 transition-transform duration-300" />
                  <h4 className="font-sans font-bold text-slate-900 text-sm md:text-base">
                    Step {idx + 1}: {step.title}
                  </h4>
                  <p className="text-slate-500 text-xs md:text-sm mt-1 leading-relaxed">{step.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="tuition-scholarships" className="py-24 bg-white relative">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            <div className="lg:col-span-6 space-y-6">
              ...
              <div className="overflow-hidden border border-slate-100 rounded-2xl shadow-xs">
                <table className="w-full text-left border-collapse bg-slate-50 text-xs md:text-sm">
                  <thead>
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700 bg-white">
                    <tr>
                      <td className="p-4 font-semibold text-slate-900">Faculty of Medicine</td>
                      <td className="p-4">36 Credits</td>
                      <td className="p-4 font-mono font-semibold text-brand-700">$9,500 / Yr</td>
                    </tr>
                    <tr>
                      <td className="p-4 font-semibold text-slate-900">Faculty of Engineering</td>
                      <td className="p-4">34 Credits</td>
                      <td className="p-4 font-mono font-semibold text-brand-700">$7,200 / Yr</td>
                    </tr>
                    <tr>
                      <td className="p-4 font-semibold text-slate-900">Faculty of Computing & IT</td>
                      <td className="p-4">32 Credits</td>
                      <td className="p-4 font-mono font-semibold text-brand-700">$6,800 / Yr</td>
                    </tr>
                    <tr>
                      <td className="p-4 font-semibold text-slate-900">Faculty of Business & Law</td>
                      <td className="p-4">30 Credits</td>
                      <td className="p-4 font-mono font-semibold text-brand-700">$5,900 / Yr</td>
                    </tr>
                    <tr>
                      <td className="p-4 font-semibold text-slate-900">Faculty of Arts</td>
                      <td className="p-4">28 Credits</td>
                      <td className="p-4 font-mono font-semibold text-brand-700">$4,800 / Yr</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={idx} className="bg-slate-50 border border-slate-100 p-5 rounded-2xl">
                    <h4 className="font-sans font-bold text-slate-900 text-sm md:text-base flex items-center space-x-2">
